...
# Инициализация роутера для эндпоинтов, связанных с пользователями
router = APIRouter(
    prefix="/users",
    tags=["users"],
...
# Модель для верификации пользователя при смене пароля
...
async def user_password_change(request: Request, username: str = Form(...),
    ...
    # Проверка совпадения старого пароля и изменение на новый
